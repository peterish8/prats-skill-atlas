...
If `vercel whoami` already works from a local login, keep that session and do not ask the user to re-authenticate. See `references/windows-auth-discovery.md` for the Windows discovery flow and token sync pattern.
