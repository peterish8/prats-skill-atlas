...
- **Act 1 fixes the #1 failure mode: building the wrong thing.** Codex interrogates *you* until intent is locked — no guessing at ambiguity. (This act is Matt Pocock's `grill-me`, used under MIT — see `THIRD-PARTY-NOTICES.md`.)
...
```markdown
# Plan: <task>
_Locked via grill — by Codex + <user>_
...
2. Grep the last line for the verdict:
   ...
   - `VERDICT: REVISE` → Codex decides **what's actually worth acting on** (Codex is final arbiter — Codex advises, doesn't command). Revise `PLAN_FILE`. Append `### Codex's response` to `LOG_FILE`: what changed, what was rejected, why. Increment round.
...
- **MAX_ROUNDS hit without APPROVED (deadlock):** do NOT fake convergence. List each unresolved point + Codex's counter-position; hand it to the user to break the tie. A flagged disagreement beats a false "approved."
...
- Codex is final arbiter on every REVISE — incorporate good critiques, reject bad ones *with a logged reason*. Don't cave to everything (defeats the cross-model check) and don't ignore it (defeats the point).
